      ...
      <div className="max-w-md mx-auto">
        ...
        <div className="bg-white/5 backdrop-blur-sm rounded-2xl p-8 border border-white/10">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-2">{t('register.inviteCode')}</label>
            ...
            <button type="submit" disabled={loading}
              ...
              {loading ? <><Loader2 className="w-5 h-5 animate-spin" />{t('register.submitting')}</> : t('register.submit')}
            </button>
          </form>
          <div className="mt-6 text-center">
            <p className="text-gray-400">{t('register.hasAccount')}{' '}
              <Link to="/login" className="text-indigo-400 hover:text-indigo-300">{t('register.login')}</Link>
            </p>
          </div>
        </div>
      </div>
