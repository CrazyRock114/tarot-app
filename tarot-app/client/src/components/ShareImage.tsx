...
const ShareImage = ({ visible, onClose, shareUrl, question, spreadName, readerName, interpretation, cards }: ShareImageProps) => {
  ...
  const generateImage = async () => {
    setGenerating(true);
    // Get translated strings once for canvas rendering
    const labels = {
      aiReading: t('shareImage.aiReading'),
      tarotReader: t('shareImage.tarotReader'),
      reversed: t('shareImage.reversed'),
      upright: t('shareImage.upright'),
      aiSummary: t('shareImage.aiSummary'),
      scanQR: t('shareImage.scanQR'),
      tryFortune: t('shareImage.tryFortune'),
      brandLine: t('shareImage.brandLine'),
    };
    try {
      const W = 750;
      const pad = 48;
      ...
      ctx.fillText(labels.aiReading, W / 2, curY + 18);
      curY += 30;
      ...
      if (readerName) {
        ctx.font = '22px sans-serif';
        ctx.fillStyle = '#a78bfa';
        ctx.fillText(labels.tarotReader + readerName, W / 2, curY + 18);
        curY += 24;
      }
      curY += 20;
      ...
      for (let i = 0; i < cards.length; i++) {
        ...
        ctx.fillText(card.orientation === 'reversed' ? labels.reversed : labels.upright, cx + cardW / 2, labelY + 22);
      }
      ...
      ctx.fillStyle = '#a78bfa';
      ctx.fillText(labels.aiSummary, pad + 20, boxY + 30);
      ...
      ctx.fillStyle = '#e0e7ff';
      ctx.fillText(labels.scanQR, qrX + qrSize + 24, qrY + 35);
      
      ctx.font = '22px sans-serif';
      ctx.fillStyle = '#94a3b8';
      ctx.fillText(labels.tryFortune, qrX + qrSize + 24, qrY + 70);
      
      ctx.font = '20px sans-serif';
      ctx.fillStyle = '#6366f1';
      ctx.fillText(labels.brandLine, qrX + qrSize + 24, qrY + 105);
      ...
      <div className="relative max-w-md w-full max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        {imageUrl && imageUrl !== 'error' ? (
          <div className="flex flex-col items-center gap-4">
            <img src={imageUrl} alt="share" className="w-full rounded-xl" />
            <p className="text-gray-400 text-sm text-center">{t('shareImage.longPressSave')}</p>
            <button
              onClick={() => {
            ...
            >
              ...
              {t('shareImage.saveImage')}
            </button>
          </div>
        ) : imageUrl === 'error' ? (
          <div className="flex flex-col items-center gap-4 py-12">
            <p className="text-red-400">{t('shareImage.generationFailed')}</p>
            <button onClick={onClose} className="px-4 py-2 bg-gray-700 text-white rounded-lg">{t('shareImage.close')}</button>
          </div>
        ) : (
          <div className="flex flex-col items-center gap-4 py-12">
            <Loader2 className="w-8 h-8 text-purple-400 animate-spin" />
            <p className="text-gray-400">{t('shareImage.generating')}</p>
          </div>
        )}
      </div>
    </div>
