...
const PRIZES = [
  { name: '5分',     emoji: '🪙', color: '#4f46e5' },
  { name: '10分',    emoji: '💰', color: '#7c3aed' },
  { name: '20分',    emoji: '💎', color: '#9333ea' },
  { name: '谢谢参与', emoji: '🍃', color: '#374151' },
  { name: '50分',    emoji: '🏆', color: '#db2777' },
  { name: '免费券',   emoji: '🎫', color: '#059669' },
  { name: '100分',   emoji: '👑', color: '#d97706' },
  { name: '再来一次', emoji: '🔄', color: '#2563eb' },
];
  ...
  const handleSpin = async () => {
    ...
    try {
      ...
      const map: Record<string, string> = {
        '5积分': '5分', '10积分': '10分', '20积分': '20分',
        '50积分': '50分', '100积分': '100分',
        '免费占卜券x1': '免费券', '谢谢参与': '谢谢参与',
      };
      const target = map[data.prize] || data.prize;
      let idx = PRIZES.findIndex(p => p.name === target);
      if (idx < 0) idx = 3;
        ...
        <AnimatePresence>
          {showResult && result && (
            ...
            >
              ...
              <div className="text-white font-bold text-lg">{result.prize}</div>
